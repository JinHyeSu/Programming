...
public class SwingDialogDELETERESERVATION_1LIST extends JDialog {
	private String[] colName = {"번호","시각","고객명","좌석","주문 음식","총합","예약"};
	private DefaultTableModel model = new DefaultTableModel(colName, 0);
	private JTable table = new JTable(model);
	private JPanel pan1;
	private JPanel pan2;
	public SwingDialogDELETERESERVATION_1LIST(RestaurantManager rm) {
		super();
		...
		for(int i=0;i<result.length;i++) { if(! result[i][6].equals("N")) { model.addRow(result[i]); } } //테이블 데이터 생성
				...
				if(b.getText().equals("선택")) {
					...
					if(! text.equals("") && isNum(text)) {
						...
						if(num>0 && num<=rm.listSize()) {
							SwingDialogDELETERESERVATION_2OK dialogOK = new SwingDialogDELETERESERVATION_2OK(rm,num);
	...
	} // SwingDialogDELETERESERVATION_1LIST end
